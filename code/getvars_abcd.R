...
## ---- Non-repeated -----------------------------------------------------------
non_class_abcd <- dh.classDiscrepancy("nonrep")
non_class_abcd %>% print(n = Inf)

## ---- Monthly repeated -------------------------------------------------------
month_class_abcd <- dh.classDiscrepancy("monthrep")
month_class_abcd %>% print(n = Inf)

## ---- Yearly repeated --------------------------------------------------------
year_class_abcd <- dh.classDiscrepancy("yearrep")
...
fact_repair <- tibble(
  var = c("preg_smk", "ethn3_m", "preg_ht", "areases_quint_preg", 
          "areases_tert_preg", "greenyn300_preg", "edu_m_"),
  df = c(rep("nonrep", 6), rep("yearrep", 1)))
