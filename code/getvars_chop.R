################################################################################
## Project: bmi-poc
## Script purpose: Prepare data for analysis - ABCD  
...
################################################################################
# 2. Summarise factor variables to repair  
################################################################################
fact_repair_chop <- tibble(
  var = c("preg_smk", "ethn3_m", "preg_ht", "edu_m_"),
  df = c(rep("nonrep", 3), rep("yearrep", 1)))
...
dh.dropCols(
  df = "nonrep", 
  vars = fact_repair_chop %>% dplyr::filter(df == "nonrep") %>% pull(var), 
  new_obj = "nonrep", 
  type = "remove")

dh.dropCols(
  df = "yearrep", 
  vars = fact_repair_chop %>% dplyr::filter(df == "yearrep") %>% pull(var), 
  new_obj = "yearrep", 
  type = "remove")

## ---- Join correct variables back --------------------------------------------
ds.dataFrame(
  x = c("nonrep", fact_repair_chop %>% dplyr::filter(df == "nonrep") %>% pull(var)),
  newobj = "nonrep_fix")

ds.dataFrame(
  x = c("yearrep", fact_repair_chop %>% dplyr::filter(df == "yearrep") %>% pull(var)),
  newobj = "yearrep_fix")
...
# 15. Create IQR
################################################################################
env_coh <- c("abcd", "alspac", "bib", "dnbc", "eden", "genr", "inma", "ninfea", 
             "moba", "rhea")  
...
iqr.vars <- c("ndvi300_preg_iqr_c")

## ---- Save progress ----------------------------------------------------------
datashield.workspace_save(conns["abcd"], "bmi_poc_sec_13")
...
dh.defineCases(
  df = "bmi_poc", 
  vars = c("bmi.0_730", "bmi.730_1461", "bmi.1461_2922", "bmi.2922_5113"),
  type = "any",
  new_obj = "outcome")
...
## ---- Ethnicity --------------------------------------------------------------
ds.asNumeric("fix_vars$ethn3_m", "ethnicity")
...
ds.asFactor("ethn3_m_f", "ethn3_m_f")
...
################################################################################
# 25. Finally we fill the DF  
################################################################################
ds.dataFrameFill("analysis_df", "analysis_df")

datashield.workspace_save(conns, "bmi_poc_sec_24_a")
################################################################################
# 26. Create dataset of excluded participants
################################################################################
ds.dataFrameSubset(
  df.name = "bmi_poc", 
  V1.name = "valid_case", 
